...
#define v8si_bisearch() \
    { \
        ...
        _mm256_maskstore_epi32(&mx_type_val(high)[0], mx_type_reg(mask), mx_type_reg(mid)); \
        mx_type_reg(mask) =  _mm256_xor_si256(mx_type_reg(mask), _mm256_set1_epi32(~0)); \
        _mm256_maskstore_epi32(&mx_type_val(low)[0], mx_type_reg(mask), mx_type_reg(mid)); \
    }
