    ...
    for (k = 0; k < mstr_chunks_in_height(rhs); k += 1) {
        for (j = 0; j < mstr_chunks_in_width(rhs); j += 1) {
            ...
            for (i = 0; i < mstr_chunks_in_height(lhs); i += 1) {
                ...
                ssel = mx_ceil_to_multiples_of_8(rchk_cols) / 8 - 1 + (uint32_t)(lchk_cols == I32S_IN_CACHE_LINE);
                dsel = mx_ceil_to_multiples_of_8(dchk_cols) / 8 - 1 + (uint32_t)(dchk_cols == I32S_IN_CACHE_LINE);
